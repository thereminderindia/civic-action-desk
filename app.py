...
if st.button("🚀 1. Generate Official Letter"):
    if user_phone and (not user_phone.isdigit() or len(user_phone) != 10):
        ...
        st.error("⚠️ Please provide your Name, Location, and Issue.")
    else:
        with st.spinner(f"Processing in {target_language}..."):
            ...
            - SENDER: {user_name}, Pincode: {pincode if pincode else 'GPS Detected'}
            - {f"CONTACT: {user_phone}" if user_phone else ""}
            - DATE: {current_date}
            - RECIPIENT: Municipal Authorities of the area. (If 247775, use Kandhla, UP).
            - BODY: Urgent 3-paragraph letter.
            ...
            (Letter content)
            [LETTER_END]
            [ENGLISH_SUMMARY]: (English translation)
